...
export default function RouteCard({ route, rank }: Props) {
  const isTop = rank === 0;

  return (
    <div
      className={`bg-white rounded-2xl border overflow-hidden ${
        isTop ? 'border-sky-200 shadow-md' : 'border-gray-100 shadow-sm'
      }`}
    >
      {/* Header */}
      <div
        className={`px-5 py-4 border-b ${
          isTop ? 'bg-sky-50 border-sky-100' : 'bg-gray-50 border-gray-100'
        }`}
      >
                ...
                {route.tags.map(tag => (
                  <span
                    ...
                    className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                      TAG_CONFIG[tag]?.className ?? 'bg-gray-100 text-gray-600'
                    }`}
                  >
